...
class NewsDetailViewModel @AssistedInject constructor(
    ...
    @Assisted val articleId: String,
) : ViewModel() {

    val uiState: StateFlow<NewsDetailUiState> = newsFeedRepository.observeArticle(articleId)
        .map { article ->
            if (article != null) {
                NewsDetailUiState.Success(article)
            } else {
    ...
    @AssistedFactory
    interface Factory {
        fun create(articleId: String): NewsDetailViewModel
    }
}
